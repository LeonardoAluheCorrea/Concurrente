...
public class Pasajero implements Runnable {
    ...
    public void run() {
        ...
        //Ahora realizara el check-in
        Hall hall = aeropuerto.getHallEspera();
        //Se dirije al hall de espera para luego realizar el check-in
        boolean realizo = false;
        while (!realizo) {
            if (p.accederPuesto(nombre)) {//Si puede acceder al puesto de check in
                p.comenzarCheckIn(nombre);//Se acerca para hacer el check in
                try {
                    Thread.sleep(2000); //Es atendido 
                } catch (InterruptedException ex) {
                    Logger.getLogger(Pasajero.class.getName()).log(Level.SEVERE, null, ex);
                }
                p.terminarCheckIn(nombre);//Termina el check in y se va
                realizo = true;
            } 
            else { //Si no puede acceder espera en el hall
        ...
        boolean subio = false;
        while (!subio){//Mientras no haya podido subir al tren
            ...
            subio = t.subirse();
            if (!subio){
                System.out.println(nombre + " no ha podido subir al tren, espera al proximo recorrido");
            }
        }
        ...
        System.out.println(nombre + " espera a la llamada para embarcar su vuelo, el " + vuelo.getNro());
        if (vuelo.yaDespego()){
            System.out.println("El vuelo " + vuelo.getNro() + " que debia tomar " + nombre + " ya ha despegado, lo perdio");
        }
        else{    
            vuelo.abordar();
            System.out.println(nombre + " abordo el vuelo " + vuelo.getNro());
        }
    }
